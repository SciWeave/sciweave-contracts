...
contract AccessToken is BaseToken {
    ...
    mapping(uint256 => AccessTokenMetadata) tokenMetadatas; // mapping from token id to metadata
    ...
    function mint(address _to, string calldata _cid) external onlyDao returns (uint256) {
        ...
        AccessTokenMetadata memory _metadata = AccessTokenMetadata(dao, _to, _cid);
    ...
    function getTokenMetadata(
        uint256 _tokenId
    ) external view returns (AccessTokenMetadata memory) {
